...
def plot_confusion_matrix(cm, classes, 
    ...
    plt.savefig("Confusion_Matrix", dpi=150, bbox_inches="tight"); plt.close()
...
misclassified_idx = np.where(y_predict != y_true)[0]
...
if len(misclassified_idx) == 0: 
    ...
    plt.imshow(img)
    plt.title(f"True: {CLASS_NAMES[y_true[i]]}, Predicted: {CLASS_NAMES[y_predict[i]]}")
    plt.axis("off")
